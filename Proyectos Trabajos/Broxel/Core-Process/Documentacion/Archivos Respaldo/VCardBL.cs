...
namespace wsBroxel.App_Code.VCBL
{
    ...
    public class VCardBL
    {
        ...
        {
            var emailValido = Regex.IsMatch(email, @"^((?:(?:[a-zA-Z0-9_\-\.]+)@(?:(?:\[[0-9]{1,3}\.[0-9]{1,3}\.[‌​0-9]{1,3}\.)|(?:(?:[‌​a-zA-Z0-9\-]+\.)+))(‌​?:[a-zA-Z]{2,4}|[0-9‌​]{1,3})(?:\]?)(?:\s*‌​;\s*|\s*$))*)$");
            var celularValido = Regex.IsMatch(celular, "^[0-9]+$");
            var clabeValida = Regex.IsMatch(celular, "^[0-9]+$");

            if (idComercio >= 0 && (emailValido || string.IsNullOrEmpty(email)) && (celularValido || string.IsNullOrEmpty(celular)) && (clabeValida || string.IsNullOrEmpty(clabe)))
            {
            ...
            {
                return "Existio un problema al dar de alta al comercio " + idComercio.ToString(CultureInfo.InvariantCulture) + " ya que los parametros enviados no son validos."
