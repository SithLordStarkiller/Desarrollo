...
using wsBroxel.PetrusDb.BackOfficce;
...
namespace wsBroxel
{
    ...
    public class BroxelService : WebService
    {
        ...
        private NIPResponse CambiarNip(NIPRequest request, string host)
        {
            ...
            var tarjetaCuenta = Helper.GetTarjetaFromTarjeta(request.Tarjeta.NumeroTarjeta);
            if (tarjetaCuenta == null || String.IsNullOrEmpty(tarjetaCuenta.Cuenta))
            {
                response.CodigoRespuesta = 999;
                response.UserResponse = "No se encontró la tarjeta destino.";
                response.NumeroAutorizacion = "-1";
                response.Success = 0;
                return response;
            }
            String numCuenta = tarjetaCuenta.Cuenta;
        ...
        private NIPResponse CambiarNipNew(NIPRequest request, string host)
        {
            ...
            var numCuenta = tarjetaCuenta.Cuenta;
		...
		private NominarCuentaResponse Nominar(NominarCuentaRequest request, string numCuenta)
        {
            ...
            var procesador = Helper.GetProcesadorFromCuenta(numCuenta);
            ...
            switch (procesador)
            {
                ...
                case 2:
                    ...
                    var backOffice = new BackOfficeContext();

                    var broxelContex = new broxelco_rdgEntities();

                    var estadoCredencial = broxelContex.estadosCredencial;

                    var generalAccountNumber = Convert.ToInt64(numCuenta);
                    ...
                    string document;
                    short? documentTypeCode;
                    string email;
                    short? gender;
                    long? phoneNumber;

                    if (request.Cuenta != null && request.Cuenta.Direccion != null)
                    {
                        countryCode = 484;
                        //geographicCode = backOffice.TGD_GEO_STATE.FirstOrDefault(x => x.ISO_CODE == broxelContex.estadosCredencial.FirstOrDefault(y => y.Estado.Contains(@"/" + request.Cuenta.Direccion.Provincia + "/")).ISO_CODE).ISO_CODE;
                        neighbourhood = request.Cuenta.Direccion.Barrio;
                        cityCode = backOffice.TGD_CITY.Where(x => x.TGD_GEO_STATE == backOffice.TGD_GEO_STATE.FirstOrDefault(y => y.ISO_CODE == geographicCode)).FirstOrDefault(x => x.CITY_NAME.Contains(@"/" + request.Cuenta.Direccion.Zona + "/")).ID_CITY;
                    }

                    if (request.Cuenta.Tarjetas != null && request.Cuenta.Tarjetas.TarjetasCredencial.Count > 1)
                    {
                        birthDate = request.Cuenta.Tarjetas.TarjetasCredencial.FirstOrDefault(x => x.Persona != null).Persona.FechaNacimiento;
                        birthPlace = null;
                        celNumber = Convert.ToInt64(request.Cuenta.Telefonos.FirstOrDefault().Numero);
                        customerName = request.Cuenta.Tarjetas.TarjetasCredencial.FirstOrDefault().Persona.Nombre;
                        customerSurname = "";
                        document = request.Cuenta.Documento.Numero;
                        documentTypeCode = backOffice.TGD_DOCUMENT_TYPE.Where(x => x.ID_DOCUMENT_TYPE == 1).FirstOrDefault(x => x.DESCRIPTION.Contains(@"/" + request.Cuenta.Documento.Tipo + "/")).ID_DOCUMENT_TYPE;
                        email = request.Cuenta.Emails.FirstOrDefault().mail;
                        gender = (short?)(request.Cuenta.Tarjetas.TarjetasCredencial.FirstOrDefault().Persona.Sexo == "F" ? 1 : 2);
                        phoneNumber = Convert.ToInt64(request.Cuenta.Telefonos.FirstOrDefault().Numero);
                    }

                    var datosCuenta = new CustomerService.NominateAccountCustomerDTO
                    {
                        Address = new CustomerService.NominateAccountAddressDTO
                        {
                            CityCode = cityCode, 
                            CountryCode = countryCode,
                            DoorNumber = request.Cuenta.Direccion.Numero,
                            GeographicCode = geographicCode,
                            Neighbourhood = request.Cuenta.Direccion.Localidad,
                            Street = request.Cuenta.Direccion.Calle,
                            StreetType = null
                        },
                        ...
                        Document = request.Cuenta.Documento.Numero,
                        DocumentTypeCode = backOffice.TGD_DOCUMENT_TYPE.Where(x=>x.ID_DOCUMENT_TYPE == 1).FirstOrDefault(x=>x.DESCRIPTION == request.Cuenta.Documento.Tipo).ID_DOCUMENT_TYPE,
                        Email = request.Cuenta.Emails.FirstOrDefault().mail,
                        Gender = (short?)(request.Cuenta.Tarjetas.TarjetasCredencial.FirstOrDefault().Persona.Sexo == "F" ? 1:2),
                        PhoneNumber = Convert.ToInt64(request.Cuenta.Telefonos.FirstOrDefault().Numero)
                    };

                    var response = dis.NominateAccountCustomer("103", generalAccountNumber,datosCuenta);

                    NominarCuentaResponse nominarPetrus = new NominarCuentaResponse
                    {
                        CodigoRespuesta = Convert.ToInt32(response.Errors),
                        UserResponse = ""
                    };
                    ...
                    return nominarPetrus;

                default:

                    NominarCuentaResponse nominarError = new NominarCuentaResponse
                    {
                        //CodigoRespuesta = Convert.ToInt32(TarjetaArray.First().ReturnCode),
                        //UserResponse = TarjetaArray.First().ReturnMessage,
                        //Success = TarjetaArray.First().ReturnCode == "-1" ? 1 : 0,
		...
		private LimiteResponse ReversoLimiteNewBL(LimiteRequest request, Movimiento mov)
		{
			var r = new LimiteResponse();
			try
			{
        ...
        */
        ...
        private LimiteResponse SetPagoOri(CargoRequest request)
        {
            BroxelEntities _ctx = new BroxelEntities();
            LimiteResponse response = new LimiteResponse();
        ...
        public CargoResponse SetCargo(Decimal monto, String nombreTarjeta, String numeroTarjeta, String cvc2, String fechaExpira, String numReferencia, String nombreReferencia, Int32 idUsuario, Int32 idComercio)
        {
            ...
            if (monto >= 0.01M)
            {
                var tarjetaCuenta = Helper.GetTarjetaFromTarjeta(numeroTarjeta);
                if (tarjetaCuenta == null)
                {
                    return new CargoResponse
                    {
                ...
                CargoRequest request = new CargoRequest
                {
                    ...
                    Tarjeta = new Tarjeta(nombreTarjeta, numeroTarjeta, fechaExpira, cvc2, tarjetaCuenta.Cuenta),
                    Cargo = new Cargo
                    {
                        Monto = Convert.ToDecimal(monto),
                        IdUsuario = idUsuario,
        ...
        public LimiteResponse EfectuarPago(Decimal monto, String nombreTarjeta, String numeroTarjeta, String cvc2, String fechaExpira, Int32 idUsuario, Int32 idComercio, String cuenta)
        {
            if (monto >= 0.01M)
            {
                ...
                if (response.Success == 1)
                    new GenericBL().ContarPagos(cuenta, monto, idUsuario.ToString());
    ...
    "yMQ3E3ert6", "Corporativos Broxel");
        ...
        public TransferenciaResponse TransferenciaDeCuentas(String cuentaOrigen, Decimal monto, String tarjetaDestino, String emailSolicitante, String tipo, Decimal porcentaje = 0)
        {
            ...
            if (monto < 0.0M)
                ...
                {
                    CodigoRespuesta = 999,
                    UserResponse = "No es posible transferir montos negativos",
                    Comision = comision,
                };
