...
namespace srvBroxel
{
    ...
    public class RollingXmlWriterTraceListener : XmlWriterTraceListener
    {
        ...
        /// The current numeric suffix for trace file names
        /// </summary>
        private string currentFileSuffixDate = "01-01-1900";
        ...
        /// <summary>
        /// Attribute for windows event Log
        /// </summary>
            ...
            : base(filename)
        {
            ...
            this.currentFileSuffixDate = this.GetTraceFileDate();
        ...
        public string CurrentTraceFileName
        {
            get
            {
                var fileNumber = this.currentFileSuffixNumber > 0 ? this.currentFileSuffixNumber.ToString(CultureInfo.InvariantCulture) : "";
                return Path.Combine(
                    Path.GetDirectoryName(this.basicTraceFileName),
                    Path.GetFileNameWithoutExtension(this.basicTraceFileName)
                    + "_" + this.currentFileSuffixDate + fileNumber 
                    +  Path.GetExtension(this.basicTraceFileName));
            }
        }
        ...
        protected bool IsRollingConditionReached
        {
            get
            {
                // Compare sysdate to file date

                if (DateTime.Now.ToString("dd-MM-yyyy").Equals(this.currentFileSuffixDate))
                {
                    return false;
                }
                else
        ...
        private void StartNewTraceFile()
        {
            try
            {
                ...
                this.currentFileSuffixDate = GetTraceFileDate();
                ...
                {
                    //_eventLog.WriteEntry("Error al crear archivo nuevo: " + e.Message + ", forzando creación");
                    this.currentFileSuffixNumber = 1;
                    while (File.Exists(this.CurrentTraceFileName))
                    {
                        this.currentFileSuffixNumber++;
                    }
                    this.Writer = new StreamWriter(new FileStream(this.CurrentTraceFileName, FileMode.Append));
                }
                File.Delete(this.basicTraceFileName);
            }
            catch (Exception e)
            {
                //_eventLog.WriteEntry("Error al insertar nuevo archivo " + this.CurrentTraceFileName + ": " + e.Message,EventLogEntryType.Error);
            }
        }
        ...
        /// <summary>
        /// Gets the trace file date by calculating current date 
        /// </summary>
        /// <returns>Current date in dd-MM-YYYY format</returns>
        private string GetTraceFileDate()
        {
        ...
        private void InitEventLog()
        {
            try
            {
                ...
                if (!EventLog.SourceExists("CreditosServiceSource"))
                {
                    EventLog.CreateEventSource(
                        "CreditosServiceSource", "CreditosServiceLog");
                }
                _eventLog.Source = "CreditosServiceSource";
                _eventLog.Log = "CreditosServiceLog";
            }
            catch
            {
