Ui.prototype.init = function () {
    ...
    this.urls = {
        ...
        entidadFederativa: this.getUrl('/Catalogo/FactorEntidadFederativaConsulta'),
    };
    this.controls = {
        buttons: {
    ...
    var succesEntidadFederativa = function (data) {
        ...
        if (lisrEntidad.length > 0) {
            $.each(lisrEntidad, function (index, value) {
                var listEstado = value.Estados;
                if (listEstado.length > 0) {
                    $.each(listEstado, function (index, valueEstado) {
                        var estado = {
                            Identificador: valueEstado.Identificador,
                            Nombre: valueEstado.Nombre
                        };
