Ui.prototype.init = function () {
    ...
    $(this.controls.buttons.buscar).click(function () {
        if ($("#GastosInherentes").val() == "") {
            $("#IdGastosInherente").val(0);
        }
        var data = {
            ObjectResult: {
                IsActive: $("input[name=estatus]:checked").val(),
                Identificador: $("#IdGastosInherente").val()
            }, Paging: {
...
$(document).ready(function () {
    ...
    $(function () {
        $("#GastosInherentes").autocomplete({
            source: "/Catalogo/ObtenerGastosInherentes",
            minLength: 1,
            scroll: true,
            select: function (event, ui) {
                console.log("Selected: " + ui.item.value + " aka " + ui.item.id);
                $("#IdGastosInherente").val(ui.item.id).attr("selected", "selected").change();
                var id = ui.item.id;
                $("#IdGastosInherente").val(id);
