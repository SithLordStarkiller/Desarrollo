 Ui.prototype.init = function () {
    ...
    this.urls = {
        serviciosCapacitacion: this.getUrl('/Catalogo/ServicioCapacitacionConsulta'),
        servicioCapacitacion: this.getUrl('/Catalogo/ServicioCapacitacion'),
        guardar: this.getUrl('/Catalogo/ServicioCapacitacionGuardar'),
        buscar: this.getUrl('/Catalogo/ServicioCapacitacionConsultaCriterio'),
        documentoServicios: this.getUrl('/Solicitud/ClienteDocumentos'),
        documentoServicio: this.getUrl('/Files/ServicioFile')
    };
    this.controls = {
        grids: {
            servicioCapacitacion: new controls.Grid('divServicioCapacitacion', { maxHeight: 400, pager: { show: true, pageSize: 10, currentPage: 0 }, showPlusButton: false })
        },
        buttons: {
            create: $("#btnCreate"),
    ...
    this.controls.grids.servicioCapacitacion.init(columns);
    this.controls.grids.servicioCapacitacion.addListener("onPage", function (evt) {
        self.controls.grids.servicioCapacitacion.currentPage = evt.currentPage;
        self.load();
    });
    ...
    var success = function (data) {
        if (data.Result === 1) {
            var loadGrid = function (data) {
                self.controls.grids.servicioCapacitacion.reload([]);
                self.controls.grids.servicioCapacitacion.reload(data);
            };
    ...
    $(this.controls.buttons.buscar).click(function () {
        var data = {
            ObjectResult: {
                ...
                CurrentPage: self.controls.grids.servicioCapacitacion.currentPage,
                Rows: self.controls.grids.servicioCapacitacion.pageSize
            }
        };
    ...
    this.controls.grids.servicioCapacitacion.addListener("onButtonClick", function (evt) {
        var url = "";
        var data = {};
        var getPartial = function (data) {
            ...
            $('#btnSave').on('click', function () {
                $.validator.unobtrusive.parse("#servicioCapacitacionForm");
                if ($("#servicioCapacitacionForm").valid()) {
        ...
        switch (evt.event) {
            case 'Edit':
            case 'View':
                url = self.urls.servicioCapacitacion;
...
Ui.prototype.load = function (evt) {
    var self = this;
    var page = self.controls.grids.servicioCapacitacion.currentPage;
    var rows = self.controls.grids.servicioCapacitacion.pageSize;
    var loadGrid = function (data) {
        self.controls.grids.servicioCapacitacion.loadRows([]);
        if (data.Result) {
            self.controls.grids.servicioCapacitacion.currentPage = data.Paging.CurrentPage;
            self.controls.grids.servicioCapacitacion.pages = data.Paging.Pages;
            self.controls.grids.servicioCapacitacion.loadRows(data.List);
        }

    };
    var data = { page: page, rows: rows };

    this.SendAjax('POST', self.urls.serviciosCapacitacion, 'json', data, loadGrid);
};
