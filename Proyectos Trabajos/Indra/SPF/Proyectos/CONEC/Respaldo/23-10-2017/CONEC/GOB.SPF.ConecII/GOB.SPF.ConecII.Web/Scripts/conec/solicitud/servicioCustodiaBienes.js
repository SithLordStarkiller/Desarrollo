 Ui.prototype.init = function () {
    ...
    this.urls = {
        serviciosCustodiaBienes: this.getUrl('/Catalogo/ServicioCustodiaBienesConsulta'),
        servicioCustodiaBienes: this.getUrl('/Catalogo/ServicioCustodiaBienes'),
        instalaciones: this.getUrl('/Catalogo/ServicioCustodiaBienes'),
        guardar: this.getUrl('/Catalogo/ServicioCustodiaBienesGuardar'),
        buscar: this.getUrl('/Catalogo/ServicioCustodiaBienesConsultaCriterio')
    };
    this.controls = {
        grids: {
            servicioCustodiaBienes: new controls.Grid('divServicioCustodiaBienes', { maxHeight: 400, pager: { show: true, pageSize: 10, currentPage: 0 }, showPlusButton: false })
        },
        buttons: {
            create: $("#btnCreate"),
    ...
    this.controls.grids.servicioCustodiaBienes.init(columns);
    this.controls.grids.servicioCustodiaBienes.addListener("onPage", function (evt) {
        self.controls.grids.servicioCustodiaBienes.currentPage = evt.currentPage;
        self.load();
    });
    var columns = [{ text: 'No.', iskey: true, css: 'col-xs-1', visible: true },
    ...
    var success = function (data) {
        if (data.Result === 1) {
            var loadGrid = function (data) {
                self.controls.grids.servicioCustodiaBienes.reload([]);
                self.controls.grids.servicioCustodiaBienes.reload(data);
            };
    ...
    $(this.controls.buttons.buscar).click(function () {
        var data = {
            ObjectResult: {
                ...
                CurrentPage: self.controls.grids.servicioCustodiaBienes.currentPage,
                Rows: self.controls.grids.servicioCustodiaBienes.pageSize
            }
        };
    ...
    this.controls.grids.servicioCustodiaBienes.addListener("onButtonClick", function (evt) {
        var url = "";
        var data = {};
        var getPartial = function (data) {
            ...
            $('#btnSave').on('click', function () {
                $.validator.unobtrusive.parse("#servicioCustodiaBienesForm");
                if ($("#servicioCustodiaBienesForm").valid()) {
        ...
        switch (evt.event) {
            case 'Edit':
            case 'View':
                url = self.urls.servicioCustodiaBienes;
...
Ui.prototype.load = function (evt) {
    var self = this;
    var page = self.controls.grids.servicioCustodiaBienes.currentPage;
    var rows = self.controls.grids.servicioCustodiaBienes.pageSize;
    var loadGrid = function (data) {
        self.controls.grids.servicioCustodiaBienes.loadRows([]);
        if (data.Result) {
            self.controls.grids.servicioCustodiaBienes.currentPage = data.Paging.CurrentPage;
            self.controls.grids.servicioCustodiaBienes.pages = data.Paging.Pages;
            self.controls.grids.servicioCustodiaBienes.loadRows(data.List);
        }

    };
    var data = { page: page, rows: rows };

    this.SendAjax('POST', self.urls.serviciosCustodiaBienes, 'json', data, loadGrid);
};
