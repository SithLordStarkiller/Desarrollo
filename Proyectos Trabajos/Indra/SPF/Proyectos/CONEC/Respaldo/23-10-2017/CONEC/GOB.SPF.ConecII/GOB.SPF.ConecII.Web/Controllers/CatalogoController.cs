namespace GOB.SPF.ConecII.Web.Controllers
{
    ...
    public class CatalogoController : Controller
    {
        ...
        [HttpGet]
        [AllowAnonymous]
        public JsonResult ClasificacionYFactorObtieneEstados(UiFactorEntidadFederativa model)
        {
            try
            {
                UiPaging paging = new UiPaging { All = 1 };
                ...
                var res = (from x in result.List.ToList()
                           where x.IdClasificadorFactor == model.IdClasificadorFactor && x.IdFactor == model.IdFactor
                           select x).ToList();
        ...
        public JsonResult ObtenerFactores(UiFactorEntidadFederativa model)
        {
            try
            {
                ...
                List<UiFactorEntidadFederativa> factores = new List<UiFactorEntidadFederativa>();

                ServicesCatalog clientService = new ServicesCatalog();

                factorList.List = clientService.ObtenerFactor(1, 100000);
                factores = clientService.FactorEntidadFederativaObtenerSinAgrupar(paging);
                ...
                var ids = (from a in factores.ToList()
                                  select new
                                  {
                                      IdFactor = a.IdFactor
                                  }).ToList();

                ids.RemoveAll(d => factorList.List.Exists(b => b.Identificador == d.IdFactor));
                ...
                var Factores = (from x in factorList.List.ToList()
                                where x.IdClasificacionFactor == model.IdClasificadorFactor
                                select x).ToList();

                return Json(new { estados.List, Factores }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                return Json(new { query = "Error", JsonRequestBehavior.AllowGet });
        ...
        {
            UiResultPage<UiFracciones> DivList = new UiResultPage<UiFracciones>();

            ServicesCatalog clientService = new ServicesCatalog();

            DivList.List = clientService.ObtenerFracciones(1, 20);

            var queryF = DivList.List.Select(x => new { x.Identificador, x.Nombre }).OrderBy(x => x.Nombre).ToList();
            ViewBag.Fracciones = new SelectList(queryF, "Identificador", "Name");
        ...
        public ActionResult Cuotas()
        {
            ...
            conceptos = clientService.ObtenerCuotas(1, 20);
