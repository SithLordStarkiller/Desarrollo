...
namespace GOB.SPF.ConecII.AccessData
{
    ...
    public class RepositoryTiposDocumento : IRepository<TipoDocumento>
    {
        ...
        public IEnumerable<TipoDocumento> Obtener(IPaging paging)
        {
        ...
        public int Insertar(TipoDocumento tipoDocumento)
        {
            ...
            using (var cmd = _unitOfWork.CreateCommand())
            {
                ...
                parameters[2] = new SqlParameter() { Value = tipoDocumento.Confidencial, ParameterName = "@Confidencial" };
        ...
        public IEnumerable<TipoDocumento> ObtenerPorCriterio(IPaging paging, TipoDocumento tipoDocumento)
        {
            var result = new List<TipoDocumento>();
            ...
            using (var cmd = _unitOfWork.CreateCommand())
            {
                ...
                if(!string.IsNullOrEmpty(tipoDocumento.Nombre))
                    ...
                    cmd.Parameters.Add(new SqlParameter() { Value = DBNull.Value, ParameterName = "@Nombre" });

                cmd.Parameters.Add(new SqlParameter() { Value = paging.CurrentPage, ParameterName = "@pagina" });
                cmd.Parameters.Add(new SqlParameter() { Value = paging.Rows, ParameterName = "@filas" });
                cmd.Parameters.Add(new SqlParameter() { Value = tipoDocumento.Identificador, ParameterName = "@IdActividad" });
                ...
                using ( var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        if (!(reader["FechaInicial"] == DBNull.Value))
                            tipoDocumento1.FechaInicial = Convert.ToDateTime(reader["FechaInicial"]);
                        if (!(reader["FechaFinal"] == DBNull.Value))
                            tipoDocumento1.FechaFinal = Convert.ToDateTime(reader["FechaFinal"]);
                        tipoDocumento1.Activo = Convert.ToBoolean(reader["Activo"]);
        ...
        public string ValidarRegistro(TipoDocumento tipoDocumento)
        {
            ...
            //public IEnumerable<TipoDocumento> ObtenerPorNombre(IPaging paging, TipoDocumento entity)
            //{
            //    var result = new List<TipoDocumento>();
