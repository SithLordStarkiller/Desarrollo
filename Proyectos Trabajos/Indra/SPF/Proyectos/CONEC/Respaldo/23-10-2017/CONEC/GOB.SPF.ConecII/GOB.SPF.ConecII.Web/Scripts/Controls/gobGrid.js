...
gob.Controls.Grid.prototype.loadRows = function (rows,hierarchy,hierarchyTable,parentId) {
    ...
    if (hierarchy && (!rows || rows.length === 0)) { 
    ...
    $(tbody).find("td[class=no-data]").closest("tr").remove();
    $('.no-data', tbody).removeClass('no-data');
    ...
    for (var indexRow = 0, len = rows.length; indexRow < len; indexRow++) {
        ...
        for (var indexCol = 0, lenCols = hierarchy ? this.defaultData.detail.columns.length : this.columns.length; indexCol < lenCols; indexCol++) {
            ...
            //console.log(column.field + ' ' + dataRow.hasOwnProperty(column.field));
            //Object.getOwnPropertyNames(dataRow).forEach(function (val, idx, array) {
            //    console.log(val + " -> " + dataRow[val]);
            //});
            var value = this.getFieldValue(dataRow, column.field);
            //console.log(column.field + ' ' + value);
            //value = column.field == '' ? dataRow[column.field] : eval('dataRow.' + column.field);
            ...
            if (column.fieldValueReplacement && $.isFunction(column.fieldValueReplacement)) {
                cellValue = column.fieldValueReplacement.call(this, dataRow, cellValue, column);
            }

            cell.innerHTML = cellValue;
            if (!$.isEmptyObject(dataRow[this.defaultData.detail.field]) && column.isHierarchy) {
                ...
                $(a).click(function () {
                    if ($(this).find("span").hasClass("glyphicon-chevron-right")) {
                        ...
                        $("div", this.parentNode.parentNode.nextElementSibling).slideDown(500);
                    } else {
                        $(this).find("span").removeClass("glyphicon-chevron-down").addClass("glyphicon-chevron-right");
                        var row = this;
                        $("div", this.parentNode.parentNode.nextElementSibling).slideUp(500, function ()
                        {
                            $(row.parentNode.parentNode.nextElementSibling).removeClass("hierarchy-display").addClass("hierarchy-hide");
                        });
                    }
                });
                cell.appendChild(a);
            ...
            switch (column.cellType.type) {
                ...
                case cons.CHECKBOX:
                    ...
                    if (column.controlSettings) {
                        if (column.controlSettings.block === true)
                            check.disabled = true;

                        if (cellValue && column.controlSettings.blockTrueValue)
                            check.disabled = true;

                        if (column.controlSettings.blockByFieldValue) {
                            if (column.controlSettings.blockByFieldValue.block === true) {
                                var fieldValue = self.getFieldValue(dataRow, column.controlSettings.blockByFieldValue.fieldName);
                                if (fieldValue === column.controlSettings.blockByFieldValue.value) {
                                    check.disabled = true;
                                }
                            }
                        }
                ...
                case cons.BUTTON:
                    ...
                    switch (cssClass) {
                        ...
                        case 'Inactive':
                            ...
                            break;
                        case 'Delete':
                            spanBtn.className = "glyphicon glyphicon-trash delete ";
                            button.setAttribute('data-cmd', cssClass);
                            button.appendChild(spanBtn);
                            break;
                        case 'Location':
        ...
        if (isHierarchy) {
            ...
            for (var index = 0; index < this.defaultData.detail.columns.length; index++) {
                var thhead = document.createElement("th");
                thhead.innerHTML = this.defaultData.detail.columns[index].text;
            ...
            if (this.defaultData.detail.field !== '') {
                var neastedDiv = document.createElement('div');
                var result = self.loadRows(dataRow[this.defaultData.detail.field], true, table, trow.id);
                neastedDiv.style.display = "none";
                neastedDiv.appendChild(result);
                td.appendChild(neastedDiv);
            }
            tr.appendChild(td);
...
gob.Controls.Grid.prototype.getPage = function (position, sender) {
    ...
    var paginate = function () {
        ...
        var index = 0;
        var element = 1;
        var rows = $(self.tbody).find(".parent-tr");
        ...
        while (index < len) {
            ...
            if (!$(row).hasClass('grid-hidden-row') && $(row).hasClass('parent-tr')) {
...
gob.Controls.Grid.prototype.getFieldValue = function (obj, key) {
