 Ui.prototype.init = function () {
    ...
    this.urls = {
        serviciosProteccionPersonas: this.getUrl('/Catalogo/ServicioProteccionPersonasConsulta'),
        servicioProteccionPersonas: this.getUrl('/Catalogo/ServicioProteccionPersonas'),
        instalaciones: this.getUrl('/Catalogo/ServicioProteccionPersonas'),
        guardar: this.getUrl('/Catalogo/ServicioProteccionPersonasGuardar'),
        buscar: this.getUrl('/Catalogo/ServicioProteccionPersonasConsultaCriterio')
    };
    this.controls = {
        grids: {
            servicioProteccionPersonas: new controls.Grid('divServicioProteccionPersonas', { maxHeight: 400, pager: { show: true, pageSize: 10, currentPage: 0 }, showPlusButton: false })
        },
        buttons: {
            create: $("#btnCreate"),
    ...
    this.controls.grids.servicioProteccionPersonas.init(columns);
    this.controls.grids.servicioProteccionPersonas.addListener("onPage", function (evt) {
        self.controls.grids.servicioProteccionPersonas.currentPage = evt.currentPage;
        self.load();
    });
    var columns = [{ text: 'No.', iskey: true, css: 'col-xs-1', visible: true },
    ...
    var success = function (data) {
        if (data.Result === 1) {
            var loadGrid = function (data) {
                self.controls.grids.servicioProteccionPersonas.reload([]);
                self.controls.grids.servicioProteccionPersonas.reload(data);
            };
    ...
    $(this.controls.buttons.buscar).click(function () {
        var data = {
            ObjectResult: {
                ...
                CurrentPage: self.controls.grids.servicioProteccionPersonas.currentPage,
                Rows: self.controls.grids.servicioProteccionPersonas.pageSize
            }
        };
    ...
    this.controls.grids.servicioProteccionPersonas.addListener("onButtonClick", function (evt) {
        var url = "";
        var data = {};
        var getPartial = function (data) {
            ...
            $('#btnSave').on('click', function () {
                $.validator.unobtrusive.parse("#servicioProteccionPersonasForm");
                if ($("#servicioProteccionPersonasForm").valid()) {
        ...
        switch (evt.event) {
            case 'Edit':
            case 'View':
                url = self.urls.servicioProteccionPersonas;
...
Ui.prototype.load = function (evt) {
    var self = this;
    var page = self.controls.grids.servicioProteccionPersonas.currentPage;
    var rows = self.controls.grids.servicioProteccionPersonas.pageSize;
    var loadGrid = function (data) {
        self.controls.grids.servicioProteccionPersonas.loadRows([]);
        if (data.Result) {
            self.controls.grids.servicioProteccionPersonas.currentPage = data.Paging.CurrentPage;
            self.controls.grids.servicioProteccionPersonas.pages = data.Paging.Pages;
            self.controls.grids.servicioProteccionPersonas.loadRows(data.List);
        }

    };
    var data = { page: page, rows: rows };

    this.SendAjax('POST', self.urls.serviciosProteccionPersonas, 'json', data, loadGrid);
};
