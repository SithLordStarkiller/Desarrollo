Ui.prototype.init = function () {
    ...
    this.urls = {
        serviciosTecnologia: this.getUrl('/Catalogo/ServicioTecnologiaConsulta'),
        servicioTecnologia: this.getUrl('/Catalogo/ServicioTecnologia'),
        instalaciones: this.getUrl('/Catalogo/ServicioTecnologia'),
        guardar: this.getUrl('/Catalogo/ServicioTecnologiaGuardar'),
        buscar: this.getUrl('/Catalogo/ServicioTecnologiaConsultaCriterio')
    };
    this.controls = {
        grids: {
            servicioTecnologia: new controls.Grid('divServicioTecnologia', { maxHeight: 400, pager: { show: true, pageSize: 10, currentPage: 0 }, showPlusButton: false })
        },
        buttons: {
            create: $("#btnCreate"),
    ...
    this.controls.grids.servicioTecnologia.init(columns);
    this.controls.grids.servicioTecnologia.addListener("onPage", function (evt) {
        self.controls.grids.servicioTecnologia.currentPage = evt.currentPage;
        self.load();
    });
    var columns = [{ text: 'No.', iskey: true, css: 'col-xs-1', visible: true },
    ...
    var success = function (data) {
        if (data.Result === 1) {
            var loadGrid = function (data) {
                self.controls.grids.servicioTecnologia.reload([]);
                self.controls.grids.servicioTecnologia.reload(data);
            };
    ...
    $(this.controls.buttons.buscar).click(function () {
        var data = {
            ObjectResult: {
                ...
                CurrentPage: self.controls.grids.servicioTecnologia.currentPage,
                Rows: self.controls.grids.servicioTecnologia.pageSize
            }
        };
    ...
    this.controls.grids.servicioTecnologia.addListener("onButtonClick", function (evt) {
        var url = "";
        var data = {};
        var getPartial = function (data) {
            ...
            $('#btnSave').on('click', function () {
                $.validator.unobtrusive.parse("#servicioTecnologiaForm");
                if ($("#servicioTecnologiaForm").valid()) {
        ...
        switch (evt.event) {
            case 'Edit':
            case 'View':
                url = self.urls.servicioTecnologia;
...
Ui.prototype.load = function (evt) {
    var self = this;
    var page = self.controls.grids.servicioTecnologia.currentPage;
    var rows = self.controls.grids.servicioTecnologia.pageSize;
    var loadGrid = function (data) {
        self.controls.grids.servicioTecnologia.loadRows([]);
        if (data.Result) {
            self.controls.grids.servicioTecnologia.currentPage = data.Paging.CurrentPage;
            self.controls.grids.servicioTecnologia.pages = data.Paging.Pages;
            self.controls.grids.servicioTecnologia.loadRows(data.List);
        }

    };
    var data = { page: page, rows: rows };

    this.SendAjax('POST', self.urls.serviciosTecnologia, 'json', data, loadGrid);
};
