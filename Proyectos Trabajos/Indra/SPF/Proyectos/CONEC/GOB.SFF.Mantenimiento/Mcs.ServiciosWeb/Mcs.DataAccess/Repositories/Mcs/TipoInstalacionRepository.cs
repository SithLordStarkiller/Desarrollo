...
{
    public class TipoInstalacionRepository: Repository<BeTipoInstalacion>
    {
        private readonly DbContext _context;
        public TipoInstalacionRepository(DbContext context) : base(context)
        {
            if (context == null)
            {
        ...
        public IList<BeTipoInstalacion> GetTiposInstalaciones()
        {
            using (var command = _context.CreateCommand())
            {
                command.CommandType = CommandType.StoredProcedure;
                command.CommandText = SpsMcsResources.SpGetAllTiposInstalacionesWs;
