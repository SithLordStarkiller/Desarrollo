...
namespace GOB.SPF.ConecII.AccessData
{
    ...
    public class RepositoryCuotas : IRepository<Cuota>
    {
        ...
        public IEnumerable<Cuota> Obtener(IPaging paging)
        {
            ...
            using (var cmd = _unitOfWork.CreateCommand())
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        cuotas.EsProducto = Convert.ToBoolean(reader["EsProducto"]);
        ...
        public IEnumerable<Cuota> ObtenerPorCriterio(IPaging paging, Cuota entity)
        {
            ...
            using (var cmd = _unitOfWork.CreateCommand())
            {
                ...
                if (entity.EsProductoSearch != null)
                    cmd.Parameters.Add(new SqlParameter() { Value = entity.EsProductoSearch, ParameterName = "@EsProducto" });
                else
                    cmd.Parameters.Add(new SqlParameter() { Value = DBNull.Value, ParameterName = "@EsProducto" });

                cmd.Parameters.Add(new SqlParameter() { Value = entity.Ano, ParameterName = "@Ano" });
                cmd.Parameters.Add(new SqlParameter() { Value = paging.CurrentPage, ParameterName = "@pagina" });
                cmd.Parameters.Add(new SqlParameter() { Value = paging.Rows, ParameterName = "@filas" });
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        cuotas.FechaTermino = Convert.ToDateTime(reader["FechaTermino"]);
                        cuotas.FechaPublicaDof = Convert.ToDateTime(reader["FechaPublicaDof"]);
                        cuotas.FechaInicial = Convert.ToDateTime(reader["FechaInicial"]);
                        //if (!(reader["FechaFinal"] == DBNull.Value))
                        //    cuotas.FechaFinal = Convert.ToDateTime(reader["FechaFinal"]);
                        cuotas.Activo = Convert.ToBoolean(reader["Activo"]);
        ...
        public IEnumerable<Cuota> ObtenerConceptos(Cuota model)
        {
            var result = new List<Cuota>();

            using (var cmd = _unitOfWork.CreateCommand())
            {
                cmd.CommandText = Catalogos.CuotasObtenerIdTipoServicio;
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new SqlParameter() { Value = model.IdTipoServicio, ParameterName = "@IdTipoServicio" });
        ...
        public IEnumerable<Cuota> ObtenerAnos(IPaging paging)
        {
            var result = new List<Cuota>();

            using (var cmd = _unitOfWork.CreateCommand())
            {
                cmd.CommandText = Catalogos.CuotasObtenerPorCriterioAnos;
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new SqlParameter() { Value = paging.CurrentPage, ParameterName = "@pagina" });
                cmd.Parameters.Add(new SqlParameter() { Value = paging.Rows, ParameterName = "@filas" });
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        cuotas.Ano = Convert.ToInt32(reader["FechaEntradaVigor"]);
                        cuotas.Activo =true;
                        result.Add(cuotas);
                    }
                }
