...
namespace GOB.SPF.ConecII.Business
{
    public class FirmaDigital
    {
        ...
        /// </summary>
        public byte[] GenerarFirma(string cadenaComplemnto)
        {
            if (String.IsNullOrEmpty(cadenaComplemnto))
                throw new ConecException("Cadena complemento inválida");

            CadenaComplemento = cadenaComplemnto;

            byte[] certByte =  Convert.FromBase64String(Certificado);

            byte[] keyByte = Convert.FromBase64String(Llave);

            X509Certificate2 certificado = new X509Certificate2(certByte);
            ...
            {
                status = ValidaEmisorVigencia(certByte, emisor);
                if (status == 100)
                    break;
            }
            ...
            Firma = ObtenerFirma(certByte, keyByte, Password, cadenaComplemnto);
        ...
        /// <param name="razon"></param>
        /// <returns></returns>
        private byte[] ObtenerFirma(byte[] certFile, byte[] keyFile, string password, string cadenaComplemento)
        {
            ...
            return FirmaCadena(objCert, ref cadenaComplemento);
        ...
        /// <param name="strCadenaComplemento"></param>
        /// <param name="strRazon"></param>
        /// <returns></returns>
        /// <remarks>
        /// 20/10/2017
        /// Horacio Torres
        /// Agrege el RFC con el valor certificadoValidar[0].SubjectDN.GetValueList()[5].ToString()
        /// </remarks>
        private byte[] FirmaCadena(SysX509.X509Certificate2 SignerCertificate, ref string strCadenaComplemento)
        {
            ...
            string strCadenaOriginal = "||SERVICIO DE PROTECCIÓN FEDERAL|" + 
                                        DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToShortTimeString() + "|" +
                                        strCadenaComplemento + "|" +
                                        certificadoValidar[0].SubjectDN.GetValueList()[0].ToString() + "|" + // nombre
                                        certificadoValidar[0].SubjectDN.GetValueList()[5].ToString() + "|" + // rfc
                                        certificadoValidar[0].SubjectDN.GetValueList()[4].ToString() + "|" + // email
                                        SignerCertificate.SerialNumber + "|" + 
                                        SignerCertificate.NotBefore + "|" + 
                                        SignerCertificate.NotAfter + "||";
