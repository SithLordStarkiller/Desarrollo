...
gob.Controls.Grid.prototype.loadRows = function (rows,hierarchy,hierarchyTable) {
    var self = this;
    var doc = document;
    var cons = gob.Constants.cellType;
    ...
    for (var indexRow = 0, len = rows.length; indexRow < len; indexRow++) {
        ...
        if (isHierarchy) {
            ...
            if (this.defaultData.detail.field !== '') {
                var result = self.loadRows(dataRow[this.defaultData.detail.field], true, table);
...
gob.Controls.Grid.prototype.getPage = function (position, sender) {
    ...
    var paginate = function () {
        ...
        self.pages = Math.ceil((len - 1) / pageSize) === 0 ? 1 : Math.ceil((len - 1) / pageSize);
        if (sender === undefined)
