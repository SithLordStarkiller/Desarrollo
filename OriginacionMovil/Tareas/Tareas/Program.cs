...
using PubliPayments.ServiceReferenceExpedienteDigital;
...
namespace PubliPayments
{
    internal class Program
    {
        ...
        public static void Run()
        {
            ...
            if (ConfigurationManager.AppSettings["TareaEnvioDocumentosExpedienteDigital"] != null &&
                ConfigurationManager.AppSettings["TareaEnvioDocumentosExpedienteDigital"].ToLower() == "true" &&
                Config.AplicacionActual().Nombre.Contains("OriginacionMovil"))
            {
                var tEnvioDocumentosExpedienteDigital = new Task(TareaEnvioDocumentosExpedienteDigital);
                tEnvioDocumentosExpedienteDigital.Start();
            }
            ...
            if (ConfigurationManager.AppSettings["TareaEnvioOrdenes"]          != null &&
                ConfigurationManager.AppSettings["TareaEnvioOrdenes"].ToLower() == "true")
            {
                var procesarOrdenesFallidas = new Task( EnviarOrdenesFallidas );
                procesarOrdenesFallidas.Start();
            }
        ...
        private static void TareaRecibirArchivos()
        {
            ...
            _archivoProcesar = (ConfigurationManager.AppSettings["ArchivoAsignacion"]
                ...
                //+ ";" + ConfigurationManager.AppSettings["ArchivoReasignaciones"]
                //+ ";" + ConfigurationManager.AppSettings["ArchivoPagos"]  
                ).Split(';');
        ...
        private static void TareaEnvioDocumentosExpedienteDigital()
        {
            while (ejecutandose)
            {
                var listaTareas = new List<Task>();
                try
                {
                    const string sql = "exec OrdenesPorGenerarPDF";
                    var conexion = ConexionSql.Instance;
                    var cnn = conexion.IniciaConexion();
                    var sc = new SqlCommand(sql, cnn);
                    var sda = new SqlDataAdapter(sc);
                    ...
                    {
                        var orden = row["idOrden"].ToString();
                        var tarea = new Task(() => GeneraPdfOriginacion(orden));
                        tarea.Start();
                        listaTareas.Add(tarea);
                        Thread.Sleep(1);
                    }
                ...
                {
                    Logger.WriteLine(Logger.TipoTraceLog.Error, 1, "Tareas-GeneraPdfOriginacion-TareaEnvioDocumentosExpedienteDigital",
                      "Error en GeneraPdfOriginacion-TareaEnvioDocumentosExpedienteDigital: " + ex.Message);
                }
                Task.WaitAll(listaTareas.ToArray());
                Thread.Sleep(120000);
            }
        ...
        private static void GeneraPdfOriginacion(string orden)
        {
            try
            {
                var usuario = string.Empty;
                var nombreAcreditado = string.Empty;
                var entidadFinanciera = string.Empty;
                var archivo = "UnificadoOriginacion";
                var guid = string.Empty;
                var intentos = 0;

                Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas-GeneraPdfOriginacion","Orden : " + orden);

                var urlPdf = new GeneraPdf().GeneraPdfCompletoOriginacion(Convert.ToInt32(orden), archivo);
                if (urlPdf.StartsWith("Error:"))
                    throw new Exception(urlPdf);

                Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas-GeneraPdfOriginacion", "UrlPdf: " + urlPdf + " Orden : " + orden);

                string sqlUrl = string.Format("EXEC ActualizaUrlOrdenPDF @idOrden = '{0}', @urlPdf = '{1}';", orden, urlPdf);
                var conexion = ConexionSql.Instance;
                var cnn = conexion.IniciaConexion();
                var sc = new SqlCommand(sqlUrl, cnn);
                var sda = new SqlDataAdapter(sc);
                var ds = new DataSet();
                sda.Fill(ds);
                conexion.CierraConexion(cnn);

                Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas-GeneraPdfOriginacion", "Datos obtenidos Orden : " + orden);

                var credito = ds.Tables[0].Rows[0]["num_Cred"].ToString();
                
                foreach (DataRow registro in ds.Tables[1].Rows)
                {
                    switch (registro["Nombre"].ToString())
                    {
                        case "Nombre":
                            nombreAcreditado = registro["Valor"].ToString();
                            break;
                        case "EntidadFinanciera":
                            entidadFinanciera = registro["Valor"].ToString();
                            break;
                        case "AssignedTo":
                            usuario = registro["Valor"].ToString();
                            break;
                        case "Intentos":
                            intentos = int.Parse(registro["Valor"].ToString());
                            break;
                        case "Token":
                            guid = registro["Valor"].ToString();
                            break;
                    }
                }

                archivo = archivo.Contains(".pdf") ? archivo.Substring(0, archivo.Length - 4) : archivo;

                Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas-GeneraPdfOriginacion", "Archivo : " + archivo + " Orden : " + orden);
                ...
                {
                    Credito = credito,
                    IdEntidadFinanciera = entidadFinanciera,
                    NomArchivo = archivo,
                    NombreAcreditado = nombreAcreditado,
                    Ruta = urlPdf.Replace("//","/").Replace(":/","://"),
                ...
                if (intentos >= 10)
                {
                    Logger.WriteLine(Logger.TipoTraceLog.Error, 1, "Tareas-GeneraPdfOriginacion", orden + "|" + "Respuesta ExpDig : " + response.Codigo + " Mensaje : " + response.Mensaje + " Orden : " + orden);
                }

                Logger.WriteLine(Logger.TipoTraceLog.Trace, 1, "Tareas-GeneraPdfOriginacion", "Respuesta ExpDig : " + response.Codigo + " Mensaje : "+response.Mensaje + " Orden : " + orden);

                if (response.Codigo == 1)
                {
                    var ordenesActualizadas = new EntOrdenes().ActualizaEstatusOrdenes(orden, 9, false, false);
                    if (ordenesActualizadas < 1)
                        Logger.WriteLine(Logger.TipoTraceLog.Error, 0, "Tareas-GeneraPdfOriginacion", "Se guardo el documento pero no se pudo actualizar la orden a su nuevo estatus, Orden: " + orden);
                }

                Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas-GeneraPdfOriginacion", "Terminado Orden : " + orden);
                
                cliente.Close();
            }
            catch (Exception ex)
            {
                Logger.WriteLine(Logger.TipoTraceLog.Error, 0, "Tareas-GeneraPdfOriginacion", 
                       "Error en GeneraPdfOriginacion : " + ex.Message + " Orden : " + orden);
            }
                    ...
                    : (tipoArchivo == 7)
                        ...
                        : (tipoArchivo == 8)
                            ...
                            : (tipoArchivo == 9)
                                ...
                                : (tipoArchivo == 10) 
                                    ? ArchivoGestionesP 
                                    :(tipoArchivo == 11) ? ArchivoRegistroAsesores: "NONE.TXT").Replace("TXT", "zip"),
        ...
        private static void ImportarArchivo(string archivo, Archivos ar)
        {
            ...
            try
            {
                            ...
                            : (archivo)))
                {
                    ...
                    //if (ar.Archivo.ToUpper().Contains(ConfigurationManager.AppSettings["ArchivoAsignacion"].ToUpper()))
                    ...
                    //else if (
                    //    ar.Archivo.ToUpper()
                    //        .Contains(ConfigurationManager.AppSettings["ArchivoComplementaria"].ToUpper()))
                    //{
                    //    tipoArchivo = 2;
                    //}
                    //else if (
                    //    ar.Archivo.ToUpper()
                    //        .Contains(ConfigurationManager.AppSettings["ArchivoActualizacion"].ToUpper()))
                    //{
                    //    tipoArchivo = 3;
                    //}
                    //else if (
                    //    ar.Archivo.ToUpper()
                    //        .Contains(ConfigurationManager.AppSettings["ArchivoReasignaciones"].ToUpper()))
                    //{
                    //    tipoArchivo = 4;
                    //}
                    //else if (
                    //    ar.Archivo.ToUpper()
                    //        .Contains(ConfigurationManager.AppSettings["ArchivoPagos"].ToUpper()))
                    //{
                    //    tipoArchivo = 5;
                    //}
                    ...
                    if (header != null)
                    {
                        ...
                        //        var temp = cabecero[i];
                        //        int anterior = 0;
                        //        for (var j = i + 1; j < cabecero.GetUpperBound(0) + 1; j++)
                        //        {
                        //            if (cabecero[j].Contains(temp))
                        //            {
                        //                anterior = j;
                        //            }
                        //        }
                        //        if (anterior != 0)
                        //        {
                        //            var temp2 = cabecero[i];
                        //            cabecero[i] = cabecero[anterior];
                        //            cabecero[anterior] = temp2;
                        //        }
                        //    }
                        ...
                        //Valida cabecero
                        //var camposTabla = context.ObtenerCamposTabla("London").ToList();
                        var camposTabla = context.ObtenerCamposTabla("Creditos").ToList();
            ...
            int[] tamaño, int tipoArchivo, ConcurrentQueue<int> cancelaciones)
        {
            ...
            //if (tipoArchivo == 5)
            //{
            //    if (line.Contains("SINSOL") || line.Contains("PAGTRN"))
            //        return "";
            //}
            ...
            for (var i = 0; i < cabecero.GetUpperBound(0) + 1; i++)
            {
                ...
                //if (cabecero[i].ToLower().Equals("num_cred"))
            ...
            try
            {
                ...
                using (var context = new SistemasCobranzaEntities())
                {
                    ...
                    var resultado = context.InsUpdLondon(idArchivo, credito, campos, valores, tipoArchivo).ToList();
                    if (resultado.Count > 0)
                    {
                        ...
                        //#region tipoArchivo 4

                        //if (tipoArchivo == 4)
                        //{
                        //    var ordenCtx = context.Ordenes.Where(x => x.num_Cred == credito && x.Estatus != 5);
                        //    foreach (var orden in ordenCtx)
                        //    {
                        //        cancelaciones.Enqueue(orden.idOrden);
                        //        orden.Estatus = 5;
                        ...
                        //    catch (Exception ex)
                        //    {
                        //        return credito + " <- " + "Error:  La orden asociada al crédito " + credito +
                        //               " no pudo ser cambiado el estatus." + ex.Message;
                        //    }
                        //}
                        ...
                        //if (tipoArchivo == 5)
                        //{
                        //    var ordenCtx = context.Ordenes.FirstOrDefault(x => x.num_Cred == credito);
                        //    if (line.Contains("REA_0") || line.Contains("R0A_0") || line.Contains("SOLPAR"))
                        //    {
                        //        if (ordenCtx != null)
                        //        {
                        //            if (ordenCtx.Estatus == 4)
                        //            {
                        //                ordenCtx.Estatus = 6;
                        //                ordenCtx.FechaModificacion = DateTime.Now;
                        //            }
                        //            else
                        //            {
                        //                if (ordenCtx.idUsuario != 0)
                        //                {
                        //                    cancelaciones.Enqueue(ordenCtx.idOrden);
                        //                }
                        //                ordenCtx.Estatus = 7;
                        //                ordenCtx.FechaModificacion = DateTime.Now;
                        //            }
                        ...
                        //            {
                        //                return credito + " <- " +
                        //                       "Error:  Error al intentar cambiar el estatus de la orden " +
                        //                       ordenCtx.idOrden + " en pagado" + ex.Message;
                        //            }
                        //        }
                        //        else
                        //        {
                        ...
                        //            catch (Exception ex)
                        //            {
                        //                return credito + " <- " + "Error:  Error al intentar crear orden a London" +
                        //                       ex.Message;
                        //            }
                        //        }
                        //    }
                        //    else if (line.Contains("MGC_04") || line.Contains("MGC_05"))
                        //    {
                        //        if (ordenCtx != null)
                        //        {
                        //            if (ordenCtx.idUsuario != 0)
                        //            {
                        //                if (ordenCtx.Estatus == 3 || ordenCtx.Estatus == 4)
                        //                {
                        //                    context.BorrarRespuestas(ordenCtx.idOrden);
                        //                } //esta en dispositivo, pero no esta gestionada aun
                        //                cancelaciones.Enqueue(ordenCtx.idOrden);
                        //                ordenCtx.Estatus = 0;
                        //                ordenCtx.FechaModificacion = DateTime.Now;
                        ...
                        //                {
                        //                    return credito + " <- " +
                        //                           "Error:  Error al intentar cambiar estatus a no asignada.(se volverá a reasignar)" +
                        //                           ex.Message;
                        //                }
                    ...
                    {
                        return credito + " <- " + "Error:  La insersión no regresó ningún resultado";
                    }
                }
            }
            catch (Exception ex)
        ...
        private static string SubeArchivosSftp(string nombreArchivo)
        {
            ...
            int tipoArchivo = nombreArchivo.Contains(ArchivoGestiones.Replace("TXT", "zip"))
                ? 6
                : nombreArchivo.Contains(ArchivoFotos.Replace("TXT", "zip"))
                    ? 7
                    : nombreArchivo.Contains(ArchivoReferencias.Replace("TXT", "zip"))
                        ? 8
                        : nombreArchivo.Contains(ArchivoRUnificado.Replace("TXT", "zip"))
                            ? 9
                            : nombreArchivo.Contains(ArchivoGestionesP.Replace("TXT", "zip")) 
                            ? 10
                            : nombreArchivo.Contains(ArchivoRegistroAsesores.Replace("TXT", "zip")) ? 11 :  0;
                        ...
                        : (tipoArchivo == 8)
                            ...
                            : (tipoArchivo == 9)
                                ...
                                : (tipoArchivo == 10) 
                                    ? ArchivoGestionesP 
                                    : (tipoArchivo == 11) 
                                    ? ArchivoRegistroAsesores : "NONE.TXT").Replace("TXT", "zip");
        ...
        private static ArchivoExportado GeneraGestionBuilder(int tipoArchivo)
        {
            ...
            //var listaCampos = new List<CampoDato>();
            var archivoExportado = new ArchivoExportado
            {
                Nombre = tipoArchivo == 6 ? ArchivoGestiones : ArchivoGestionesP,
                Contenido = null,
                Registros = 0
            };
            ...
            try
            {
                ...
                using (var client = new WebClient())
                {
                    ...
                    if (!File.Exists(path + @"\" + filename))
                    {
                        ...
                        Logger.WriteLine(Logger.TipoTraceLog.Log, 0, "Tareas", "DescargaImagenOrden - Imagen Descargada - " + orden + " - " + url);
                        
                        RotarImagen(path + @"\" + filename);

                        Logger.WriteLine(Logger.TipoTraceLog.Log, 0, "Tareas", "DescargaImagenOrden - Rotar OK - " + orden + " - " + url);
                        
                        File.Move(path + @"\" + filename, path + @"\" + nombre + Path.GetExtension(path + @"\" + filename));

                        Logger.WriteLine(Logger.TipoTraceLog.Log, 0, "Tareas", "DescargaImagenOrden - Renombrada OK - " + orden + " - " + url);

                        new EntGestionadas().ActualizaRutaImagenes(orden, campo, (url1 + @"/" + nombre + Path.GetExtension(url1 + @"/" + filename)));
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas", "DescargaImagenOrdenFlock - " + orden + " - " + url);
            ...
            using (var client = new MyClient())
            {
                ...
                {
                    Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas", "DescargaImagenOrdenFlock No disponible - " + orden + " - " + url);
                    disponible = false;
                }
            }
            ...
            try
            {
                ...
                using (var client = new WebClient())
                {
                    ...
                    {
                        Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas",
                            "DescargaImagenOrdenFlock Descargando - " + directorio + url);
                        ...
                        Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "Tareas", "DescargaImagenOrdenFlock Existe - " + path + @"\" + filename);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Rota un archivo de imagen JPG 90 grados a la izquierda solo si es mas ancho que alto
        /// </summary>
        /// <param name="archivo">Path con la ruta de archivo</param>
        private static void RotarImagen(String archivo)
        {
            using (Image imagen = Image.FromFile(archivo))
            {
        ...
        private static void EnviarOrdenesFallidas()
        {
            var dtmHoraInicio = ConfigurationManager.AppSettings["HoraEnvio"];

            while (ejecutandose)
            {
                var dtmHoraSistem = DateTime.Now.ToString("HH:mm");
                //Console.WriteLine("HoraInicial {0} Hora Final {1}", dtmHoraInicio, dtmHoraSistem );
                if (dtmHoraInicio == dtmHoraSistem )
                    new OrdenesFallidas().EnviarOrdenesCorreo();
