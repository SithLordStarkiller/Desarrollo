...
// ReSharper disable once InconsistentNaming
        ...
        private static string _connectionString = ConfigurationManager.ConnectionStrings["SqlDefault"].ConnectionString;
            ...
            String password, String email)
        {
            ...
            try
            {
                ...
                SqlCommand sc = new SqlCommand("InsertaUSuario", cnn);

                SqlParameter[] parametros = new SqlParameter[7];

                parametros[0] = new SqlParameter("@idPadre", SqlDbType.Int);
                parametros[0].Value = idPadre;
                parametros[1] = new SqlParameter("@idDominio", SqlDbType.Int);
                parametros[1].Value = idDominio;
                parametros[2] = new SqlParameter("@idRol", SqlDbType.Int);
                parametros[2].Value = idRol;
                parametros[3] = new SqlParameter("@Usuario", SqlDbType.NVarChar, 50);
                parametros[3].Value = usuario;
                parametros[4] = new SqlParameter("@Nombre", SqlDbType.NVarChar, 30);
                parametros[4].Value = nombre;
                parametros[5] = new SqlParameter("@Password", SqlDbType.NVarChar, 130);
                parametros[5].Value = password;
                parametros[6] = new SqlParameter("@Email", SqlDbType.NVarChar, 50);
                parametros[6].Value = email;
        ...
        public DataSet ObtenerGestion(String idOrden)
        {
            ...
            try
            {
                var query = "select o.*, c.CV_DESPACHO, c.CV_CREDITO, c.TX_NOMBRE_DESPACHO, c.CV_PROVEEDOR, c.CV_CONTRATO, c.CV_RUTA from Ordenes o WITH (NOLOCK)"
                            + "join Creditos c WITH (NOLOCK)"
                            + "on o.num_Cred = c.CV_CREDITO "
            ...
            {
                Logger.WriteLine(Logger.TipoTraceLog.Error, 1, "ObtenerGestion", ex.Message);
            }
            CierraConexion(cnn);
            return ds;
        }
