...
namespace PubliPayments.Entidades
{
    public class EntGestionadas
    {
        ...
        public int ObtenerOrdenCredito(string credito)
        {
            var sql = "select idOrden from Ordenes where num_cred='"+credito+"'";
            var conexion = ConexionSql.Instance;
            var cnn = conexion.IniciaConexion();
            var sc = new SqlCommand(sql, cnn);
            var sda = new SqlDataAdapter(sc);
        ...
        public DataRowCollection BorrarImagen(int idOrden,string fotosABorrar)
        {
