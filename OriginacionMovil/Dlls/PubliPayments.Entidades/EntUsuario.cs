...
namespace PubliPayments.Entidades
{
    public class EntUsuario
    {
                ...
                foreach (var colum in dataRow.Table.Columns)
                {
                    ...
                    switch (columnName.ToLower())
                    {
                        ...
                        case "idpadre":
                            ...
                            break;
                        default:
                            modelUsuario.Extra[modelUsuario.Extra.Length] = columnName;
                            break;
                    }
                }
            }
        ...
        public bool CambiarDatosUsuarioPoridUsuario(int idUsuario, string nombre, string email, string password, int Estatus)
        {
            var instancia = ConexionSql.Instance;
            var cnn = instancia.IniciaConexion();
            try
            {
                var sc = new SqlCommand("ActualizarDatosUsuario", cnn);
                var parametros = new SqlParameter[5];
