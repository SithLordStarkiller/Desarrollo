...
namespace PubliPayments.Negocios
{
    public class Respuesta
    {
            ...
            try
            {
                ...
                try
                {
                    if (idOrden < 0)
                    {
                        ...
                        if (modelOrden == null)
                        {
                            var ordenCreada = new BuscarOrdenes().CrearOrden(numCredito, -111, -110, idUsuarioLog, -1, 0);
                            if (ordenCreada < 0)
                            {
                ...
                if (orden != null)
                {
                    if (resp == null || aplicacion.ToUpper().Contains("ORIGINACIONMOVIL") || aplicacion.ToUpper().Contains("MYO"))
                    {
                        ...
                        if (credito != null)
                        {
                            ...
                            if (orden.idUsuario != 0 || esCallCenter)
                            {
                                ...
                                if (formulario != null)
                                {
                                    ...
                                    {
                                        FlujoAlternoRespuestas(dicRespuestaFinal, orden.num_Cred);
                                        try
                                        {
                                            foreach (KeyValuePair<string, string> respuestaTemp in dicRespuestaFinal)
                                            {
                                                ...
                                                if (respuesta.Key.ToUpper() == "PRODUCTID" ||
                                                    ...
                                                    respuesta.Key.ToUpper() == "DICAPLICASMS" ||
                                                respuesta.Key.ToUpper() == "LEYENDAVALIDACION") continue;
                                                ...
                                                var valorResp = (limpiaNoPermitidos(respuesta.Value.Length > 350
                                                ...
                                                context.Respuestas.Add(new Respuestas
                                                {
                                                    ...
                                                    idOrden = idOrden,
                                                    Valor = ((aplicacion.ToUpper().Contains("ORIGINACIONMOVIL")) ? (respuesta.Key.StartsWith("Foto") ? valorResp : valorResp.ToUpper()) : valorResp),
                                    ...
                                    try
                                    {
                                        orden.Tipo = !tieneSMS && !codigoCelSMS ? (esCallCenter ? "C" : " ") : (esCallCenter ? ("C" + orden.Tipo).Trim() : orden.Tipo);
                                        ...
                                        /*Enviar gestión*/
                                        if (isProduction && aplicacion.ToUpper() == "LONDON" && !actualizaCelSMS && !codigoCelSMS && nombreFormulario.ToUpper().Contains("COBSOCIAL"))
                                            Gestiones.EnviarGestion(idOrden, usuario, "");

                                        if (codigoCelSMS)
                                        {
                                            string codigo = dicRespuesta[CodigoSMS];
                                            ProcesarCodigoSMS(idOrden, codigo);
                                        }
                                        ...
                                        if (esCapturaWeb && orden.idUsuario > 0)
                                        {
                    ...
                    else if (esCallCenter || orden.Tipo.ToUpper().Contains("C")) /* se aplica proceso para que las respuestas se almacenen provisionalmente  en otra tabla y se mande la cancelación */
                    {
                        ...
                        {
                            var respPendientes = ordenNegocio.InsertarRespuestasPendientes(orden.idOrden, esCallCenter ? -111 : orden.idUsuarioAnterior, valoresRespuesta);
        ...
        private void FlujoAlternoRespuestas(Dictionary<string, string> dic, string credito)
        {
            try
            {
                if (dic == null)
                ...
                Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "FlujoAlternoRespuestas", "Credito : " + credito + " Flujo alterno Credito : " + credito);

                if (dic.ContainsKey("CodRef"))
                {
                    Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "FlujoAlternoRespuestas", "Credito : " + credito + " CodReferencia : " + dic["CodRef"]);
                    DReferenciaNumeroCredito.EjecutaRefNumCredito(credito, "INSERT", dic["CodRef"]);
                }
            }
            catch (Exception ex)
            {
                var exceptionInner = ex.InnerException != null ? ex.InnerException.Message : "";
                Logger.WriteLine(Logger.TipoTraceLog.Error, 0, "FlujoAlternoRespuestas", "Credito : " + credito + " Error : " + ex.Message + (exceptionInner != "" ? " Inner " + exceptionInner : ""));
            }
        }
