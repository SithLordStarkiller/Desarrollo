...
namespace PubliPayments.Entidades.Originacion
{
    public static class  EntDatosProspecto
    {
        ...
        public static DataSet obtenerDatosRegistro(string nss, string plazo,string centralObrera,int borrarCredito=0)
        {
            string sql = "exec obtenerDatosRegistro " +
                         " @nss=N'"+nss+"',"+
                         " @plazo=N'" + plazo + "'," +
                         " @centObrera=N'"+centralObrera+"'," +
                         " @borrarCredito="+borrarCredito;
        ...
        public static string ObtenerEntidadPorIdOrden(string idOrden)
        {
            string sql = "select Valor from Respuestas r left join CamposRespuesta cr on cr.idCampo=r.idCampo where cr.Nombre ='EntidadFinanciera' and idORden="+idOrden+" ";
            var conexion = ConexionSql.Instance;
            var cnn = conexion.IniciaConexion();
            var sc = new SqlCommand(sql, cnn);
            var sda = new SqlDataAdapter(sc);
