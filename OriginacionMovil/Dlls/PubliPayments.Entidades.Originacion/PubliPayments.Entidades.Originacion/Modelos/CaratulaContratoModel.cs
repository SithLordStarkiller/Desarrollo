...
namespace PubliPayments.Entidades.Originacion
{
    public class CaratulaContratoModel
    {
        ...
        //inicia Nuevos campos

        public string CostoAnualTotal { get; set; }

        public string ComisionXApertura { get; set; }

        public string ComisionXPrepago { get; set; }

        public string ComisionXCobranza { get; set; }

        public string MetodologiaCalculoInteres { get; set; }

        public string CondFinancierasNumero { get; set; }

        public string CondFinancierasMonto { get; set; }

        public bool? PermisoParaMercado { get; set; }

        public string PeriodicidadEdoCta { get; set; }

        public string ConsultaViaInternet { get; set; }

        public string DatosInscripcionRegAdhesion { get; set; }

        //Finaliza nuevos campos
        ...
        {
            Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "ObtenerCaratulaContratoModel", "Store Procedure");
            try
            {
        ...
        {
            Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "CaratulaContrato", "Llenando modelo");
            ...
            try
            {
                if (dataSet.Tables.Count > 0 && dataSet.Tables[0].Rows.Count > 0)
                {
                    foreach (var colum in dataSet.Tables[0].Columns)
                    {
                        ...
                        switch (columnName)
                        {
                            //comienza campos no encontrados
                            case "CostoAnualTotal":
                                modelo.CostoAnualTotal = dataSetString(dataSet, columnName);;
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta CostoAnualTotal");
                                break;
                            case "ComisionXApertura":
                                modelo.ComisionXApertura = dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta ComisionXApertura");
                                break;
                            case "ComisionXPrepago":
                                modelo.ComisionXPrepago = dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta ComisionXPrepago");
                                break;
                            case "ComisionXCobranza":
                                modelo.ComisionXCobranza = dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta ComisionXCobranza");
                                break;
                            case "MetodologiaCalculoInteres":
                                modelo.MetodologiaCalculoInteres= dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta MetodologiaCalculoInteres");
                                break;
                            case "CondFinancierasNumero":
                                modelo.CondFinancierasNumero = dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta CondFinancierasNumero");
                                break;
                            case "CondFinancierasMonto":
                                modelo.CondFinancierasMonto = dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta CondFinancierasMonto");
                                break;
                            case "PermisoParaMercado":
                                modelo.PermisoParaMercado = null;
                                break;
                            case "PeriodicidadEdoCta":
                                modelo.CostoAnualTotal = dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta PeriodicidadEdoCta");
                                break;
                            case "ConsultaViaInternet":
                                modelo.ConsultaViaInternet = dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta ConsultaViaInternet");
                                break;
                            case "DatosInscripcionRegAdhesion":
                                modelo.DatosInscripcionRegAdhesion = dataSetString(dataSet, columnName);
                                if (string.IsNullOrEmpty(modelo.TasaInteres))
                                    throw new Exception("CaratulaContratoModel Falta DatosInscripcion en regimen ahesion");
                                break;
                                //Finaliza campos no encontrados
                            case "TasaInteres":
                            ...
                            case "NumeroCredito":
                                ...
                                {
                                    throw new Exception("CaratulaContratoModel Falta NumeroCredito");
                                }
                                break;
                        }
                    }
