...
namespace PubliPayments.Entidades.Originacion
{
    public class ReciboTarjetaModel
    {
        ...
        {
            Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "ObtenerReciboTarjetaModel", "Store Procedure");
        ...
        public static ReciboTarjetaModel LlenarModelo(DataSet dataSet)
        {
            ...
            Logger.WriteLine(Logger.TipoTraceLog.Trace, 0, "ReciboTarjetaModel", "Llenando modelo");
            try
            {
                if (dataSet.Tables.Count > 0 && dataSet.Tables[0].Rows.Count > 0)
                {
                    foreach (var colum in dataSet.Tables[0].Columns)
                    {
                        ...
                        switch (columnName)
                        {
                            ...
                            case "nombreBanco":
                                modelo.Banco = DataSetString(dataSet, columnName);
                                break;
                        }
                    }
                }
