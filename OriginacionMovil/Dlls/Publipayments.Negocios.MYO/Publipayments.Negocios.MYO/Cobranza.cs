    ...
    public class Cobranza
    {
        ...
        public void ObtenerSAFCarteraVencida()
        {
            ...
            if (adeudos != null)
            {
                ...
                Trace.WriteLine(String.Format("{0} - Negocios.MYO.Cobranza_ObtenerSAFCarteraVencida, creditos {1}", DateTime.Now, creditos));
                ...
                foreach (var result in adeudos)
                {
                    InsCarteraVencida(Convert.ToInt64(result.Num_Credito), result.COD_CLIENTE, result.Cod_Agencia,
                        ...
                        result.DIA_ULT_PAGO, result.CASA, result.CELULAR, result.OFICINA, result.DES_CORREO);
                }
            }
            else
            {
                Trace.WriteLine(String.Format("{0} - Negocios.MYO.Cobranza_ObtenerSAFCarteraVencida, No hay informacion", DateTime.Now));
            }
            Logger.WriteLine(Logger.TipoTraceLog.Trace, 1, "Negocios.MYO.Cobranza", "Fin... ObtenerCarteraVencida");
                  ...
                  , string email)
        {
            ...
            //    var email = Convert.ToString(ds.Tables[0].Rows[0]["email"]);
                ...
                //if (frecuencia!=null)
                //{
            entCobranza.InsCarteraVencida("", numCredito, codCliente, codAgencia, codEmpresa, nomCliente, numPagosAtrasados, monDeduccion, diaUltPago, casa, celular, oficina, email, "semanal");
                //}
            //}
            //else
            //{
